...
class _MyHomePageState extends State<MyHomePage>
  ...
  @override
  void initState() {
    if (widget.clinic == 'عيادة داخلية') {
      widget.clinic = "عيادة باطنية";
      print(widget.clinic);
    }
    getClinics();
    // getUser();
...
// on below line we have specified camera position
  static final CameraPosition _AAUP = const CameraPosition(
    target: LatLng(32.4616388813, 35.298453802),
    zoom: 14.4746,
  );
...
// created method for getting user current location
  ...
  // static const CameraPosition _dnehadClinic = CameraPosition(
  //     //  bearing: 192.8334901395799,
  //     target: LatLng(32.46462, 35.293858),
  //     // tilt: 59.440717697143555,
  //     zoom: 19.151926040649414);

  Future<void> _goToTheClinic(CameraPosition cameraPosition) async {
        ...
        body: Stack(
          ...
          children: [
            Padding(
              ...
              child: GoogleMap(
                //     polygons: Polygon.setOptions,
                initialCameraPosition: _AAUP,
                ...
                //   onMapCreated: onCreated, mapToolbarEnabled: true
                // ),
              ),
              // Container(
                            ...
                            itemBuilder: (context, index) {
                              return Expanded(
                                child: LocationListTile(
                                  ...
                                  _markers.add(Marker(
                                      markerId: MarkerId('$index+2'),
                                      position: LatLng(
                                          destinationlist[index].lat,
                                          destinationlist[index].lng),
                                      infoWindow: InfoWindow(
                                        title: destinationlist[index].name,
                                      )));
                                  _goToTheClinic(CameraPosition(
                                      target: LatLng(destinationlist[index].lat,
                                          destinationlist[index].lng),
                                      zoom: 19.151926040649414));
                                  setState(() {});
                                }),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Text(
              'يتوجب عليك الذهاب الى ${widget.clinic}',
              style: TextStyle(
                  color: KColor.greyColor, fontFamily: 'Lalezar', fontSize: 17),
            ),
            Text(
...
class LocationListTile extends StatelessWidget {
  ...
  LocationListTile(
      this.head, this.destance, this.num, this.onTap, this.onTapGPS);
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      child: ClipRect(
            ...
            child: Row(
              ...
              children: [
                Row(
                  children: [
                    GestureDetector(
                      onTap: onTapGPS as VoidCallback,
                      child: CircleAvatar(
                          backgroundColor: Colors.white,
                          child: Text(
                            '$num',
